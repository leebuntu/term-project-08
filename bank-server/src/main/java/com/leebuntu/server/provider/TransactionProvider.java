...
public class TransactionProvider {
        private static final Database transactionDB = DatabaseManager.getDB(Constants.TRANSACTION_DB_NAME);

        public static boolean addTransaction(Transaction transaction) {
                String typeName = AccountProvider
                                .getAccountType(transaction.getSenderAccountNumber() == "ATM"
                ...
                QueryResult result = transactionDB.execute(query, transaction.getSenderId(),
        ...
        public static List<Transaction> getTransactions(String accountNumber) {
                ...
                if (result.getQueryStatus() != QueryStatus.SUCCESS
                                && result.getQueryStatus() != QueryStatus.NOT_FOUND) {
                        return new ArrayList<>();
                }

                while (result.next()) {
                        transactions.add(new Transaction((int) result.getCurrentRow().get(1),
                                        (int) result.getCurrentRow().get(2),
